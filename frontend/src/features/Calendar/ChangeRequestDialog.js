...
const ChangeRequestDialog = ({ event, open, onClose }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["change-requests"] }); // Opcjonalnie, jeśli mamy listę zgłoszeń
      onClose();
    },
  });
  ...
  const handleSubmit = () => {
    if (!event) return;
    mutation.mutate({
      course_event_id: event.id.split("-")[1], // Zakładając format `courseId-eventId`
