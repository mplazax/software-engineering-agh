...
def add_data():
    ...
    # Users
    admin = User(email="admin@example.com", password=get_password_hash("admin123"), name="Admin", surname="User", role=UserRole.ADMIN)
    ...
    coordinator = User(email="koord@example.com", password=get_password_hash("koord123"), name="Coord", surname="Person",
    ...
    # Course events
    event1 = CourseEvent(course_id=course1.id, room_id=room1.id, start_datetime=now + timedelta(days=1),
                         end_datetime=now + timedelta(days=1, hours=2), canceled=False)
    event2 = CourseEvent(course_id=course2.id, room_id=room2.id, start_datetime=now + timedelta(days=2),
                         end_datetime=now + timedelta(days=2, hours=1, minutes=30), canceled=False)
    ...
    proposal1 = AvailabilityProposal(change_request_id=req1.id, user_id=student.id,
                                     available_start_datetime=now + timedelta(days=1, hours=3),
                                     available_end_datetime=now + timedelta(days=1, hours=5))
    proposal2 = AvailabilityProposal(change_request_id=req1.id, user_id=teacher.id,
                                     available_start_datetime=now + timedelta(days=1, hours=4),
                                     available_end_datetime=now + timedelta(days=1, hours=6))

    session.add_all([proposal1, proposal2])
    session.flush()

    # Change recommendation
    recommendation1 = ChangeRecomendation(change_request_id=req1.id, recommended_start_datetime=now + timedelta(days=2),
                                          recommended_end_datetime=now + timedelta(days=2, hours=2),
